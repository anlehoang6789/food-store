...
export default function ChangePasswordForm() {
  ...
  const changePasswordV2Mutation = useUpdatePasswordV2Mutation();

  const onSubmit = async (data: ChangePasswordBodyType) => {
    // if (changePasswordMutation.isPending) return;
    if (changePasswordV2Mutation.isPending) return;
    try {
      // const result = await changePasswordMutation.mutateAsync(data);
      const result = await changePasswordV2Mutation.mutateAsync(data);
      ...
      handleErrorApi({ setError: form.setError, error });
    }
  };
